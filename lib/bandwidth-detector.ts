...
// Liberia: Export optimized image URL
export function getOptimizedImageUrl(url: string, quality?: number): string {
  const detector = BandwidthDetector.getInstance();
  const info = detector.getBandwidthInfo();
  
  if (!url) return url;
  
  // Add width parameter for responsive images
  const width = info.profile === 'low' ? 480 : 
                info.profile === 'medium' ? 768 : 1200;
  
  // Add quality parameter
  const imgQuality = quality || detector.getImageQuality();
