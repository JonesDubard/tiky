...
//   // ✅ Parse quantities synchronously
...
//           console.error("Tickets API failed:", res.status, errorBody)
//           return
//         }
...
//     // Route to correct API based on payment method
//     const apiRoute =
//       selectedMethod === "card"
...
//         console.error("Payment API error:", res.status, errorBody)
//         throw new Error(errorBody?.message || errorBody?.error || "Payment failed")
//       }

//       const result = await res.json()

//       // Card → go to Stripe processing page
//       if (selectedMethod === "card" && result.clientSecret) {
//   router.push(
//     `/checkout/card-processing?clientSecret=${result.clientSecret}&orderId=${result.orderId}`
//   )
//   return
// }

//       // Mobile money → go to success page directly (simulated)
...
export default function CheckoutPage() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // Card → go to card processing page with clientSecret + quantities
      if (selectedMethod === "card" && result.clientSecret) {
        router.push(
      ...
      // Mobile money → go to success page
      if (result.redirectUrl) {
        router.push(result.redirectUrl)
        return
      }
