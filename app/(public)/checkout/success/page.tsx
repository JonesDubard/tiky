...
//   // Replace the useEffect in success/page.tsx
// useEffect(() => {
//   if (!orderId) {
//     setLoading(false)
//     return
...
//       // If tickets not yet ready and we haven't retried too many times, try again
//       if (data.tickets.length === 0 && attempts < maxAttempts) {
//         attempts++
//         setTimeout(fetchOrder, 2000)
//       } else {
//         // Always stop loading once we have order data, tickets or not
//         setLoading(false)
//       }
//     } catch (err) {
//       console.error("Error fetching order:", err)
//       setLoading(false) // This help to stop loading on error too
//     }
//     const res = await fetch(`/api/orders/${orderId}`)
// if (!res.ok) throw new Error("Failed to fetch order")
// const data = await res.json()

// console.log("Order data received:", JSON.stringify(data, null, 2)) // ✅ Add this
// console.log("Tickets count:", data.tickets?.length) // ✅ Add this
...
//     setPdfLoading(true)
//     try {
//       const res = await fetch(`/api/tickets/pdf/${ticketId}`)
//       if (!res.ok) throw new Error("PDF generation failed")
//       const blob = await res.blob()
//       const url = URL.createObjectURL(blob)
//       const a = document.createElement("a")
//       a.href = url
//       a.download = `tiky-ticket-${ticketId}.pdf`
//       a.click()
...
//       `*Ticket ID:* ${ticket.id}\n\n` +
//       `View & download your ticket at: ${window.location.origin}/tickets/${ticket.id}`
//     )
...
//             Payment confirmed via {methodLabel}
//           </p>
//         </div>
...
//               </svg>
//             </div>
//             <p className="text-yellow-800 font-medium">Generating your tickets...</p>
...
//                     <p className="font-mono text-sm text-gray-700 font-semibold tracking-wider break-all">
//                       {ticket.id}
//                     </p>
//                   </div>
...
//                       ${ticket.ticketType.price.toFixed(2)} USD
//                     </span>
//                   </div>
//                 </div>
//               </div>
...
//                 <span className="text-gray-800">{methodLabel}</span>
//               </div>
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        if (data.tickets.length > 0) {
          // ✅ Tickets ready — stop polling
          setLoading(false)
          return
        }

        if (attempts === 0 && method === "card") {
          // First attempt with no tickets — call confirm-card as fallback
          // This handles the case where card-processing didn't call it
          console.log("No tickets found, calling confirm-card fallback...")
          try {
        ...
        if (attempts < maxAttempts) {
          ...
          // Gave up polling — show whatever we have
          setLoading(false)
        }
      } catch (err) {
        console.error("Error fetching order:", err)
