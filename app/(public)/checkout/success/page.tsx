...
function SuccessInner() {
  ...
  useEffect(() => {
    ...
    let attempts = 0
    const maxAttempts = 15 // Increased for MoMo processing time
    let isStopped = false
    ...
    const fetchOrder = async () => {
      ...
      try {
        ...
        // LOGIC: A ticket is "Ready" if it is PAID and has a QR Image generated
        const ticketsReady = data.tickets.length > 0 && 
                           data.tickets.every(t => t.status === "PAID" && t.qrImage)
        ...
        // If it's a card payment and first attempt, trigger the manual confirmation
        if (attempts === 0 && method === "card") {
          fetch("/api/payment/confirm-card", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ orderId }),
          }).catch(err => console.error("Card confirmation failed:", err))
        }

        // Keep polling if not ready
        if (attempts < maxAttempts) {
          attempts++
          setTimeout(fetchOrder, 3000) // Poll every 3 seconds
        } else {
          setLoading(false)
        }
      } catch (err) {
  ...
  const handleWhatsAppShare = (ticket: TicketInstance) => {
    ...
    const message = encodeURIComponent(
      `Ticket for ${event.title}\nID: ${ticket.id}\nView: ${window.location.origin}/tickets/${ticket.id}`
    )
    window.open(`https://wa.me/?text=${message}`, "_blank")
  }
  ...
  const paymentLabel = order.payments[0]?.paymentMethod || method || "Mobile Money"
  const ticketsArePending = order.tickets.length === 0 || order.tickets.some(t => t.status === "RESERVED")
  const currentTicket = order.tickets[activeTicket]
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500">Method: {paymentLabel}</p>
        </div>

        {ticketsArePending ? (
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-orange-100 text-center">
            <div className="animate-pulse flex flex-col items-center">
              <div className="w-48 h-48 bg-gray-100 rounded-2xl mb-4" />
              <p className="text-orange-600 font-semibold text-lg">Generating your QR Codes...</p>
              <p className="text-gray-400 text-sm">Please don't close this page. We are finalizing your secure tickets.</p>
            </div>
          </div>
        ) : (
          <>
              ...
              <div className="flex gap-2 mb-4 overflow-x-auto">
                {order.tickets.map((t, i) => (
                  <button
                    key={t.id}
                    onClick={() => setActiveTicket(i)}
                    className={`px-4 py-2 rounded-full text-sm font-bold ${activeTicket === i ? 'bg-orange-500 text-white' : 'bg-white text-gray-500'}`}
                  >
                    Ticket {i + 1}
                  </button>
                ))}
            ...
            {currentTicket && (
              <div className="bg-white rounded-3xl shadow-xl overflow-hidden mb-6 border border-gray-100">
                <div className="bg-orange-500 p-6 text-white">
                  <p className="text-xs opacity-80 uppercase tracking-widest">{currentTicket.ticketType.name}</p>
                  <h2 className="text-xl font-bold">{currentTicket.ticketType.event.title}</h2>
                </div>
                
                <div className="p-8 text-center">
                  {currentTicket.qrImage ? (
                    <img src={currentTicket.qrImage} alt="QR Code" className="w-48 h-48 mx-auto border-4 border-gray-50 rounded-xl" />
                  ) : (
                ...
                <div className="p-6 border-t border-dashed border-gray-100 space-y-3">
                  <button onClick={() => handleDownloadPDF(currentTicket.id)} className="w-full bg-gray-900 text-white py-3 rounded-xl font-bold">Download PDF</button>
                  <button onClick={() => handleWhatsAppShare(currentTicket)} className="w-full bg-green-500 text-white py-3 rounded-xl font-bold">WhatsApp Share</button>
                </div>
              </div>
            )}
          </>
