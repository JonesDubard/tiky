...
export async function GET(
  ...
  try {
    ...
    const poll = await prisma.poll.findUnique({
      where: { id: pollId, deletedAt: null },
      select: { pollType: true, eventId: true, requiresTicket: true },
    });
    ...
    // Token‑gated polls – existing logic
    if (poll.pollType !== "TOKEN_GATED" || !poll.eventId) {
