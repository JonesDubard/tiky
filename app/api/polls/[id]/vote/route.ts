...
export async function POST(
  ...
  try {
    ...
    // 2. Load poll (now includes voteCount on options)
    const poll = await prisma.poll.findUnique({
      where: { id: pollId, deletedAt: null },
      select: {
        id: true,
    ...
    if (poll.pollType === "PUBLIC") {
      ...
      // ── Create vote for PUBLIC poll (atomic counter increment) ───────
      ...
      const options = vote.poll.options;
      ...
      // ── Create vote linked to ticket (atomic counter increment) ───────
      const [vote] = await prisma.$transaction([
        prisma.vote.create({
          data: {
            pollId,
    ...
    return NextResponse.json(
      { error: "UNKNOWN_POLL_TYPE", message: "Invalid poll type." },
      { status: 400 }
    );
  } catch (error) {
    console.error("Vote error:", error);
