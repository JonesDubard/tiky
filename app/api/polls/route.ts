...
export async function GET(req: NextRequest) {
  try {
    ...
    const sort     = searchParams.get("sort") ?? "newest"; // 'newest' | 'oldest' | 'votes'

    const polls = await prisma.poll.findMany({
      where: {
        deletedAt: null,
        // 'live' toggle overrides to LIVE status, otherwise respect the status param
        ...(live
          ? { status: "LIVE" }
          : status !== "ALL" && { status }),
    ...
    return NextResponse.json({ polls: result, total: result.length });
  } catch (error) {
