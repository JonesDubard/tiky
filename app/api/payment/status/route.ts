...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // ─────────────────────────────────────────
    // ✅ FAST PATH: PAYMENT ALREADY COMPLETED
    // ─────────────────────────────────────────
    if (payment?.status === "COMPLETED") {
      ...
      // 🔥 Fulfill if not done yet
      console.log(`[STATUS] Running fulfillment for ${orderId}`)

      const result = await issueTicketsForOrder(orderId)

      if (!result.success) {
        console.error(`[STATUS] Fulfillment failed`, result.error)
    ...
    try {
      ...
      console.error("[STATUS] MTN error:", err)
    ...
    console.log(`[STATUS] MTN says: ${momoStatus.status}`)

    // ✅ SUCCESS
    if (momoStatus.status === "SUCCESSFUL") {
  console.log(`[STATUS] Payment SUCCESSFUL for order ${orderId}`)

  // 1️⃣ Ensure payment is marked completed
  await prisma.payment.update({
    where: { id: payment.id },
    data: {
      status: "COMPLETED",
  ...
  console.log(`[STATUS] issueTicketsForOrder result:`, result)

  if (!result.success) {
    console.error(`[STATUS] Ticket generation failed:`, result.error)

    return NextResponse.json({
      orderStatus: "PROCESSING",
      ticketsReady: false,
      message: "Payment received. Generating tickets...",
    })
  }
